...
def get_product_by_barcode(barcode: str) -> dict | None:
    ...
    try:
		# HTTP request naar de API, https://requests.readthedocs.io/en/latest/
        # timeout voorkomt dat de app blijft hangen als de API traag is
        r = requests.get(url, headers=headers, timeout=10)
        r.raise_for_status()
        data = r.json()
    except requests.RequestException:
		# Bij netwerkfouten of timeouts geven None terug
        return None
...
# Allen velden ophalen die nodig zijn
    return {
        "barcode": barcode,
